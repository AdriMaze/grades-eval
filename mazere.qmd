...
final_grades |>
  ...
  labs(
    title = "Final grade distribution by group",
    x = "Group",
    y = "Final grade"
  ) +
  theme_minimal()
```
There is a lot of heterogeneity in the distribution of final grades, and the range is also large, from around 9 to around 12. The median final grade seems to be, on average, above 10, around 11.
...
final_grades |>
  ...
  geom_boxplot() +
  labs(
    title = "Final grade distribution by group and sex",
    x = "Group",
    y = "Final grade",
    fill = "Sex"
  ) +
  theme_minimal()
```
The distributions seem to be roughly identical for make and female identically, if not, males tend to have, on average, lower final grades and higher ranges, displaying higher heterogeneity.

**Question 26**

To pass the year, a student must fulfill the following conditions:
...
pass <- avg_per_course |>
  ...
  group_by(id, group) |>
  mutate(
    pass_course = avg_grade >= 5,
    sem_avg = ave(avg_grade, semester, FUN = mean),
    pass_sem = sem_avg >= 10
  ) |>
  summarise(
    final_grade = mean(avg_grade),
    pass = all(pass_course) & all(pass_sem)
  ) |>
  ungroup()
```
...
```{r}
#| message: false
paste(pass|>filter(!pass, final_grade >= 10) |>nrow(),"students didn't pass and yet have a final grade larger or equal to 10")
```
...
pass |>
  ...
  scale_y_continuous() +
  theme_minimal() +
  labs(title = "Pass rate per group", y = "Pass rate", x = "Group")
```
...
paste("The course responsible for making a student fail owing to grades below than 5 is",responsible$course)
```
...
The success rate does not display any particular characteristic. People above 25 are very rare in themselves, so a low, albeit non-existant success rate is not a surprise.
